...
| `RelayShareBlock` | (shared) | The share/sync affordance. `variant='invite'` = prominent link/QR; `variant='waiting'` = `Syncing…`/`Synced ✓`/failed chip + demoted "Nudge X a link" + collapsed link/QR. Used by ShareScreen, RoundSummary (sender), GameOver (sender) |
